...
_HEAD_DEMO_REVISION = "demo_0016_d06_ref_profile_queue"
...
def _normalized_check_sql(value: str) -> str:
    ...
    for deparsed_form, authority_form in (
        ...
        (
            "NOTsource_fact_snapshot->>'source_qa_snapshot_digest'"
            "ISDISTINCTFROMsource_qa_snapshot_digest",
            "source_fact_snapshot->>'source_qa_snapshot_digest'"
            "ISNOTDISTINCTFROMsource_qa_snapshot_digest",
        ),
    ):
        normalized = normalized.replace(deparsed_form, authority_form)
    return normalized
...
def test_identity_local_qa_digest_separation_check_matches_orm_metadata(
    session: Session,
) -> None:
    identity_table = cast(Table, DemoSyntheticIdentity.__table__)
    orm_constraint = next(
        ...
        for constraint in identity_table.constraints
        if isinstance(constraint, CheckConstraint)
        and constraint.name == "ck_demo_synthetic_identities_d02_local_qa_digest_separation"
    )
    database_constraint = session.scalar(
        text(
            """
