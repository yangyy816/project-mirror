...
from mirror_api import demo_d02_r2_private_registry_e2 as registry
...
def test_fully_resigned_e1_epoch_identity_splice_is_rejected(tmp_path: Path) -> None:
    root = _create_root(tmp_path)
    receipt = registry.build_root_name_receipt(AUTHORITY)
    contract = registry.execution_contract_payload(AUTHORITY)
    e1_values: dict[str, registry.JsonValue] = {
    ...
    for key, value in e1_values.items():
        contract[key] = value
        if key in receipt:
            receipt[key] = value
    contract["generation_dispatch_state"] = (
        "BLOCKED_PENDING_SEPARATE_GENERATION_CAPABILITY_AUTHORITY"
    )
    receipt["registry_copy_a_id"] = "P3_P7_D02_R2_CC08_E1_REGISTRY_A"
    receipt["registry_copy_b_id"] = "P3_P7_D02_R2_CC08_E1_REGISTRY_B"
    receipt["contract_digest"] = mirror_demo_digest(
        registry.EXECUTION_CONTRACT_SCHEMA,
        contract,
    )
    receipt["receipt_digest"] = mirror_demo_digest(
        registry.ROOT_RECEIPT_SCHEMA,
...
def test_e2_registry_preserves_the_accepted_e1_schema_and_ddl_digests() -> None:
    assert (
        registry.REGISTRY_NORMALIZED_DDL_SHA256
        == "b92c9cf02e0593cdbbe9cb182e5fa230bd6a5845eef3c872b997dfa73eac9389"
    )
    assert (
        registry.REGISTRY_SCHEMA_CONTRACT_DIGEST
        == "e45bee49655805da14e26a6d9f882245be41e2931f063b357de800b0848413d0"
    )
