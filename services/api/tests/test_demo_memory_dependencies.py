...
def test_non_celery_uses_recoverable_pending_memory_boundary() -> None:
    settings = Settings(
        app_env="test",
    ...
    infrastructure = create_demo_memory_infrastructure(
        settings=settings,
        sessions=cast(async_sessionmaker[AsyncSession], object()),
    )

    assert isinstance(infrastructure.coordinator, DemoMemoryCoordinator)
    assert isinstance(infrastructure.service, DemoMemoryService)
