...
def test_default_acquisition_identities_are_exact_and_replayable() -> None:
    ...
    expected_m3_policy = "".join(
        ("e40d23b47551720fd1cd2630ac3e8bfd", "f65e8d3184e9c4b2e5127a4d09c30b09")
    )
    assert M3_PRESCREEN_POLICY_DIGEST == expected_m3_policy
    expected_qa_policy = "".join(
        ("b79d03d6a738044cca032029ba2f8461", "8c613e0539bd93ce8f19ac5dcc1cf178")
    )
    assert CANDIDATE_QA_POLICY_DIGEST == expected_qa_policy
    expected_run_key = "".join(
        ("04c4dacd3199bed812aeef542cea12b52", "1689aa58796dd2f0ea20f8a9683e1a2")
    )
    assert RUN_KEY_DIGEST == expected_run_key
    ...
    assert CANDIDATE_QA_POLICY_DIGEST == canonical_digest(
        "mirror.demo/D02CandidateQAPolicy/v1", candidate_qa_policy_payload()
    )
    ...
    assert identity.qa_policy_digest == CANDIDATE_QA_POLICY_DIGEST
    assert (
        candidate_qa_policy_payload()["legacy_measurement_qa_policy_digest"]
        == authority.RECOVERED_LEGACY_QA_POLICY_CONTENT_DIGEST
    )
