...
from mirror_api.storage_keys import internal_synthetic_raw_object_key
...
class _StoredMetadata(TypedDict):
    schema_version: str
...
class LocalSyntheticRawStorageProvider:
    """Development-only private raw storage with atomic immutable object directories."""
    ...
    def __init__(self, *, root: Path) -> None:
        ...
        self._raw_root = self._root / "internal-synthetic" / "v1" / "raw"
        self._prepare_directory(self._raw_root)
    ...
    async def store_generated_image_if_absent(
        ...
        async with self._lock:
            ...
            if existing is not None:
                ...
                return existing
            temporary = self._raw_root / (f".part-{target.name}-{secrets.token_hex(8)}")
            try:
    ...
    async def stream_generated_image(self, *, storage_reference: str) -> AsyncIterator[bytes]:
        metadata = await self.inspect_generated_image(storage_reference=storage_reference)
        if metadata is None:
            raise SyntheticStorageOperationError("synthetic_object_not_found")
        payload_path = self._target(storage_reference) / _PAYLOAD_NAME
        handle = await asyncio.to_thread(payload_path.open, "rb")
        try:
            while True:
    ...
    async def delete_generated_image(self, *, storage_reference: str) -> DeleteResult:
        self._validate_reference(storage_reference)
        async with self._lock:
            return await asyncio.to_thread(self._delete_sync, storage_reference)
    ...
    def _target(self, storage_reference: str) -> Path:
        self._validate_reference(storage_reference)
        object_key = internal_synthetic_raw_object_key(storage_reference)
        digest = object_key.rsplit("/", 1)[-1]
        target = self._raw_root / digest
        if target.parent != self._raw_root:
            raise SyntheticStorageOperationError("invalid_storage_reference")
        return target

    @staticmethod
    def _metadata_for(request: SyntheticStorageWriteRequest) -> SyntheticStoredImage:
        return SyntheticStoredImage(
            storage_reference=request.storage_reference,
            byte_size=request.payload.byte_size,
            media_type=request.payload.media_type,
            sha256=hashlib.sha256(request.payload.content).hexdigest(),
        )
    ...
    def _inspect_sync(self, storage_reference: str) -> SyntheticStoredImage | None:
        ...
        if metadata_path.is_symlink() or payload_path.is_symlink() or not payload_path.is_file():
            raise SyntheticStorageOperationError("synthetic_object_invalid")
        try:
        ...
        try:
            metadata = SyntheticStoredImage(
                storage_reference=cast(str, raw["storage_reference"]),
                byte_size=cast(int, raw["byte_size"]),
                media_type=cast(
                    Literal["image/jpeg", "image/png", "image/webp"], raw["media_type"]
                ),
            ...
            raise SyntheticStorageOperationError("synthetic_metadata_invalid") from None
        if (
            raw["schema_version"] != "mirror.synthetic-storage/raw-object/v1"
    ...
    def _write_temporary(
        ...
        document: _StoredMetadata = {
            "schema_version": "mirror.synthetic-storage/raw-object/v1",
    ...
    def _delete_sync(self, storage_reference: str) -> DeleteResult:
        target = self._target(storage_reference)
        if target.is_symlink():
            raise SyntheticStorageOperationError("synthetic_object_invalid")
        if not target.exists():
        ...
        if not target.is_dir():
            raise SyntheticStorageOperationError("synthetic_object_invalid")
        names = {entry.name for entry in target.iterdir()}
        if not names.issubset({_METADATA_NAME, _PAYLOAD_NAME}):
            raise SyntheticStorageOperationError("synthetic_object_invalid")
    ...
    def _remove_temporary(self, temporary: Path) -> None:
        if temporary.parent != self._raw_root or not temporary.name.startswith(".part-"):
