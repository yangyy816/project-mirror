...
celery_app.conf.update(
    ...
    task_routes={
        ...
        "mirror.demo_reference_profile.compile": {"queue": "mirror.demo"},
        "mirror.demo_reference_profile.reconcile": {"queue": "mirror.maintenance"},
        "mirror.demo_context.compile": {"queue": "mirror.demo"},
        "mirror.demo_context.reconcile": {"queue": "mirror.maintenance"},
        "mirror.demo_memory.rebuild": {"queue": "mirror.demo"},
        "mirror.demo_memory.reconcile": {"queue": "mirror.maintenance"},
    },
)
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="mirror.demo_editing.process",
    acks_late=True,
    reject_on_worker_lost=True,
    soft_time_limit=120,
    time_limit=150,
)
def process_demo_editing(message: dict[str, Any]) -> dict[str, str | bool | None]:
    return asyncio.run(run_demo_editing_message(message))


@celery_app.task(  # type: ignore[untyped-decorator]
    name="mirror.demo_editing.reconcile",
    autoretry_for=(RuntimeError,),
    acks_late=True,
    reject_on_worker_lost=True,
    **INGESTION_RETRY_POLICY,
)
def reconcile_demo_editing(*, limit: int = 100) -> list[str]:
    return list(
        asyncio.run(
            run_demo_editing_reconciliation(
                dispatcher=CeleryTaskDispatcher(),
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="mirror.demo_reference_profile.compile",
    acks_late=True,
    reject_on_worker_lost=True,
    soft_time_limit=240,
    time_limit=270,
)
def compile_demo_reference_profile(message: dict[str, Any]) -> dict[str, str | None]:
    return asyncio.run(run_demo_reference_profile_message(message))


@celery_app.task(  # type: ignore[untyped-decorator]
    name="mirror.demo_reference_profile.reconcile",
    autoretry_for=(RuntimeError,),
    acks_late=True,
    reject_on_worker_lost=True,
    **INGESTION_RETRY_POLICY,
)
def reconcile_demo_reference_profile(*, limit: int = 100) -> list[str]:
    return list(
        asyncio.run(
            run_demo_reference_profile_reconciliation(
                dispatcher=CeleryTaskDispatcher(),
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="mirror.demo_context.compile",
    acks_late=True,
    reject_on_worker_lost=True,
    soft_time_limit=120,
    time_limit=150,
)
def compile_demo_context(message: dict[str, Any]) -> dict[str, str | None]:
    return asyncio.run(run_demo_context_message(message))


@celery_app.task(  # type: ignore[untyped-decorator]
    name="mirror.demo_context.reconcile",
    autoretry_for=(RuntimeError,),
    acks_late=True,
    reject_on_worker_lost=True,
    **INGESTION_RETRY_POLICY,
)
def reconcile_demo_context(*, limit: int = 100) -> list[str]:
    return list(
        asyncio.run(
            run_demo_context_reconciliation(
                dispatcher=CeleryTaskDispatcher(),
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="mirror.demo_memory.rebuild",
    acks_late=True,
    reject_on_worker_lost=True,
    soft_time_limit=120,
    time_limit=150,
)
def rebuild_demo_memory(message: dict[str, Any]) -> dict[str, str | None]:
    return asyncio.run(run_demo_memory_message(message))


@celery_app.task(  # type: ignore[untyped-decorator]
    name="mirror.demo_memory.reconcile",
    autoretry_for=(RuntimeError,),
    acks_late=True,
    reject_on_worker_lost=True,
    **INGESTION_RETRY_POLICY,
)
def reconcile_demo_memory(*, limit: int = 100) -> list[str]:
    return list(
        asyncio.run(
            run_demo_memory_reconciliation(
                dispatcher=CeleryTaskDispatcher(),
...
class CeleryTaskDispatcher:
    ...
    def dispatch_demo_reference_profile(self, message: DemoReferenceProfileTaskMessage) -> str:
        message.validate()
        compile_demo_reference_profile.apply_async(
            args=[message.to_message()],
            task_id=secrets.token_hex(16),
            headers={"request_id": message.request_id, "job_id": message.job_id},
            queue="mirror.demo",
    ...
    def dispatch_demo_context(self, message: DemoContextTaskMessage) -> str:
        message.validate()
        compile_demo_context.apply_async(
            args=[message.to_message()],
            task_id=secrets.token_hex(16),
            headers={"request_id": message.request_id, "job_id": message.job_id},
            queue="mirror.demo",
    ...
    def dispatch_demo_memory(self, message: DemoMemoryTaskMessage) -> str:
        message.validate()
        rebuild_demo_memory.apply_async(
            args=[message.to_message()],
            task_id=secrets.token_hex(16),
            headers={"request_id": message.request_id, "job_id": message.job_id},
            queue="mirror.demo",
    ...
    def dispatch_demo_editing(self, message: DemoEditingTaskMessage) -> str:
        message.validate()
        process_demo_editing.apply_async(
            args=[message.to_message()],
            task_id=secrets.token_hex(16),
            headers={"request_id": message.request_id, "job_id": message.job_id},
            queue="mirror.demo",
