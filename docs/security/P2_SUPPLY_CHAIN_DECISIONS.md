...
| MediaPipe source-built zero-telemetry v1 | exact `v0.10.35` commit `f8ef212d5c962c0e853db7e59d217056b187084b`; ADR-032                                                                | Public source uses a dummy logger; Stage A patch freezes a minimal C ABI target and removes the CA-bundle path; Linux toolchain/configured closure frozen by ADR-033; Windows and model/data disposition remain independent | `STAGE_B_LINUX_OFFLINE_BUILD_AUTHORIZED`; no runtime/model approval                                | Complete offline artifact build, native/license/SBOM/vulnerability audit and exact Windows toolchain/build before execution                                                                 |
