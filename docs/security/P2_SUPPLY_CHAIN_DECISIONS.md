...
| MediaPipe source-built zero-telemetry v1 | exact `v0.10.35` commit `f8ef212d5c962c0e853db7e59d217056b187084b`; ADR-032                                                                | Public source uses a dummy logger; minimal C ABI/CA-bundle patch; Linux closure and OpenCV lock frozen; R04 portability and R05 reproducibility repairs recorded; Windows and model/data disposition remain independent | `STAGE_B_LINUX_REPRODUCIBILITY_PASS_AUDIT_IN_PROGRESS`; no runtime/model approval                  | Resolve OouraFFT distribution rights and remaining vulnerability dispositions, then exact Windows toolchain/build before execution                                                          |
...
OouraFFT functions are present in the main binary. Its notice does not clearly close modified/binary
redistribution rights, so internal isolated research may continue but distribution and production
remain blocked pending independent license judgment or removal from the closure.
