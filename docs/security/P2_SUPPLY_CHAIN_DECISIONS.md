...
| MediaPipe source-built zero-telemetry v1 | exact `v0.10.35` commit `f8ef212d5c962c0e853db7e59d217056b187084b`; ADR-032                                        | Public source uses a dummy logger; Stage A patch freezes a minimal C ABI target and removes the CA-bundle path; toolchain/build artifact and model/data disposition remain independent | `STAGE_A_PASS_BUILD_FREEZE_PENDING`; no install/runtime approval                                   | Freeze target-specific Bazel inputs, cross-platform toolchains, artifact hashes, native/license/SBOM evidence before execution                                                              |
