...
| MediaPipe source-built zero-telemetry v1 | exact `v0.10.35` commit `f8ef212d5c962c0e853db7e59d217056b187084b`; ADR-032                                                                | Public source uses a dummy logger; Windows/Linux clean reproduction and R17 SBOM/license/vulnerability dispositions pass for isolated synthetic PoC; model bundle remains private-research-only              | `STAGE_B_PASS_FOR_ISOLATED_STAGE_C`; no production/runtime/model adoption                              | Execute both-platform Stage C; any source, model, toolchain, dependency or distribution change reopens Stage B                                            |
...
| OpenCV 5.0.0 bounded M4 source closure   | `opencv/opencv` release `5.0.0`; source SHA-256 `b0528f5a...`; conditional R08 patch SHA-256 `e42a75d9...`                                 | Built `core,flann,geometry,imgproc` plus bundled static zlib 1.3.2; Apache-2.0 and retained BSD-3-Clause notices plus Zlib; both-platform reproducibility, SBOM, vulnerability and zero-egress evidence pass | `APPROVED_FOR_PRIVATE_SYNTHETIC_M4`; no project binary, production, distribution or real-user approval | Any source, patch, toolchain, module, zlib, ABI, binary digest, license, vulnerability, deterministic output, network surface or scope change reopens T04 |
...
## P2-M3-R17 hardened Linux reproduction update

Fresh output roots `clean-output-3` and `clean-output-4` independently completed the 4,597-action
R17 Linux build with `--network none`; `clean-output-1` remains failed-attempt evidence and was not
reused. Main/core/imgproc pairs are byte-identical with SHA-256 values
...
The ELF closure has a relative `$ORIGIN` RUNPATH, the required versioned Face Landmarker C exports,
and zero private-host-path, Ooura, Clearcut/certifi/CA-bundle or known network-surface string matches.
This closes cross-platform clean-build reproducibility and removes Ooura from the exact Linux closure.
Updated SBOM/license/vulnerability disposition, exact model disposition and Stage C runtime capture
remain mandatory; no dependency, model or production approval is created.

The regenerated R17 configured graph has 22,719 labels and zero `fft2d`/Ooura matches. Its
51-component CycloneDX SBOM SHA-256 is
`902088a0e70d3ce005885c01f7ee472fba19458ae803e09700df52949d152dda`; the 38-repository,
124-file license inventory SHA-256 is
`e1e77546b0a2a8148cc2f6ef6b3dc700305edad16311b09d9a836caa3c2742d3`. Grype 0.117.0 with
database v6.1.9 reports zero direct source-closure matches. The four OpenCV CPE negative-control
findings are dispositioned by the exact persistence backport plus no-crash malformed-input tests, or
by absence of the affected `objdetect`, `video` and `imgcodecs` modules/symbols.

The exact model bundle is `PRIVATE_RESEARCH_ONLY`; official component cards are Apache-2.0 while
training-data and redistribution provenance remain incomplete. Stage B therefore passes only for the
authorized isolated synthetic Stage C. Distribution, project dependency adoption, production Vision
and real-user processing remain blocked.
...
## P2-M4 OpenCV 5 bounded-source disposition

Candidate `OPENCV_5_0_0_BOUNDED_TRANSITIVE_SOURCE_V2` completed two clean Linux and two clean Windows
builds. Same-platform runtime pairs are byte-identical, cross-platform fixture pixels are identical,
all negative controls pass and the worst 1024 p95 remains below the preregistered limit. The exact
closure is OpenCV `core,flann,geometry,imgproc`, bundled static zlib `1.3.2` and the first-party
`ctypes-c-v1` wrapper.

R08's conditional no-PDB overlay changes only MSVC linker metadata. The final Windows binaries have
zero RSDS/PDB, private-path, pthread or network-import matches; Linux has only relative `$ORIGIN` and
zero network undefined symbols. Both Linux runs used `--network none`; Windows process-specific
outbound denial and Filtering Platform capture recorded zero attempted egress.

The private CycloneDX 1.6 SBOM SHA-256 is
`2345cba1a2b397f13d36d754cc83e0ba3cdd182658d48e87e84210e7ea1fa38d`. Offline Grype `0.117.0`
with database v6.1.9 reported zero matches; result SHA-256 is
`4749ee252c4d5f112399cffd05321d0d79b26740c2b3c21a4a42edafaca83e1f`.

This approves only the exact closure for private synthetic M4 adapter work after the same-SHA tracked
