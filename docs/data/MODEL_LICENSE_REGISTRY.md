...
| MediaPipe package/runtime                 | PyPI top-level Apache-2.0 claim；wheel contents unverified | N/A                                           | N/A                                  | LICENSE_REVIEW_REQUIRED / POC_BLOCKED | PRODUCTION_BLOCKED                          | 获明确下载授权后核验 exact wheels、bundled native deps、SBOM、漏洞、Python 3.13 与平台行为     |
| MediaPipe Face Landmarker artifact        | N/A                                                        | 三个官方 component model card 均标 Apache-2.0 | 仅有官方高层来源描述，逐项权利未证明 | LICENSE_REVIEW_REQUIRED / POC_BLOCKED | PRODUCTION_BLOCKED                          | 固定 GCS generation 后核验 bundle SHA-256/contents、数据权利、商业用途与再分发边界             |
...
Face Landmarker bundle 固定到 generation `1683136941468629`，但只提供 MD5/CRC32C；未获明确
artifact-download authorization 前不下载 bundle/wheel，故 bundle SHA-256、包内 notice、完整
native/transitive chain、Python 3.13 与零网络行为仍未验证。`MODEL_ARTIFACTS_ADDED: NONE`。
