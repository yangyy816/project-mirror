...
| -------------------------------------------- | -------------------------------------------------------------- | --------------------------------------------- | ------------------------------------ | ------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| MediaPipe source code                        | Apache-2.0 verified at tag `v0.10.35` / `f8ef212d...`          | N/A                                           | N/A                                  | PRIVATE SYNTHETIC M3 CLOSURE AUDITED  | PRODUCTION_BLOCKED                          | 版本或 closure 变化必须重做 source/license/SBOM/vulnerability review                               |
| MediaPipe source-built Face Landmarker C ABI | Apache-2.0 source；51-component SBOM + 38-repository inventory | separate bundle; see artifact row             | see artifact/data rows               | APPROVED FOR PRIVATE SYNTHETIC M3     | DISTRIBUTION / PRODUCTION BLOCKED           | private M3 only；不得推导 distribution、production 或 real-user authority                          |
...
| MediaPipe Face Landmarker artifact           | N/A                                                            | 三个官方 component model card 均标 Apache-2.0 | 仅有官方高层来源描述，逐项权利未证明 | PRIVATE_RESEARCH_ONLY                 | PRODUCTION_BLOCKED                          | bundle SHA-256 recorded；data rights/commercial redistribution remain unresolved                   |
...
本 registry 的 private-research disposition 不授权把 MediaPipe、OpenCV、imagededup 或未来候选安装为
项目 runtime dependency，也不得添加 `.pt`、`.pth`、`.onnx`、`.ckpt`、`.safetensors`、`.bin`、`.gguf`、
