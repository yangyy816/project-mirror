...
| --------------------------------------------------------------- | ----------------------------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| MediaPipe official wheels `0.10.35`                             | P2-M3/P3 landmark、姿态与几何输入                           | REJECTED_FOR_P2_M3_RUNTIME         | Windows Clearcut telemetry hard failure；不得用 source-built candidate 的 PASS 覆盖该失败                                 |
| MediaPipe `v0.10.35` exact-source minimal C closure             | P2-M3 V03 isolated synthetic Vision                         | POC_RUNTIME_APPROVED_PRIVATE_ONLY  | Windows/Linux clean reproduction、Stage B audit 与 Stage C zero-egress PASS；只允许 V02 Stage D，非项目依赖或生产批准     |
| OpenCV `3.4.11` minimal source overlay                          | P2-M3 V03 isolated Face Landmarker build                    | ISOLATED BUILD LOCK ONLY           | exact `core,imgproc` closure 已跨平台复现并完成漏洞处置；仅随 V03 private PoC 使用，不批准 P2-M4 runtime                  |
| OpenCV `5.0.0` / `opencv-python-headless==5.0.0.93`             | P2-M4 wheel candidate                                       | FURTHER_RESEARCH                   | determinism/performance PASS；full wheel codec/network closure 过宽且 native vulnerability Gate 未闭合，不批准 T05        |
| OpenCV `5.0.0` bounded source closure                           | P2-M4 private synthetic transform adapter                   | APPROVED_FOR_PRIVATE_SYNTHETIC_M4  | exact `core,flann,geometry,imgproc` + static zlib closure；T04 evidence only，非 package/production/distribution approval |
...
| Pillow 12.3.0                                                   | P1-M4 ingestion；P2 normalization/M4 canonical RGB boundary | APPROVED FOR PINNED BOUNDED SCOPES | 不升级版本；M4 仅 canonical JPEG decode/RGB encode，精确 wheel/hash/license/native evidence 见 adoption record            |
...
P0/P1 与 P2-M1/P2-M2/P2-M3/P2-M4 保持 `FROZEN`；P2-M5 仅开放 rolling-wave refinement，尚未获得
implementation authorization。当前 active milestone 只能按其已冻结 protocol 和 Gate 前进；本文不授权其引入未来候选。
Pillow 12.3.0 保持既有锁定版本；任务所需依赖与模型 artifact 可在 Project Owner 授权下下载到私有缓存，
但下载不等于 adoption、license、distribution、production 或 real-user processing 批准。Terra 发现新
