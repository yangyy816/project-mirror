...
| Phase 2 | COMMITTED | P2-M1–M4/M7 已冻结；P2-M5 E01 capability blocked；P2-M6/M8 entry closed     |
...
| P2-M4   | FROZEN    | repaired closure `11bda0a`；run `32169725374` 三 jobs 与七项 artifacts 全绿 |
...
Phase 2 仅处理可追溯的成年合成人物刺激资产；真实用户数据、真人图片、SelfState、问卷运行、DesiredDelta、编辑、支付和部署均不在范围。P2-M1 的已冻结规划修订、T01 权限、Principal checkpoint、研究 Gate 与停止条件见 `P2_PLANNING_AMENDMENT.md`；项目主线 P2-M5 的权威见 `P2_M5_EXECUTION_PROTOCOL.md`，并行且无碰撞的 P2-M7 运维轨道权威见 `P2_M7_EXECUTION_PROTOCOL.md`。
...
| P2-M5     | EXECUTING | CC04-B E01 contract 已接受；private output sink 与 actual-human duplicate review capability 未证明，零调用 fail closed |
| P2-M6     | COMMITTED | QuestionBank draft, immutable release and revocation；须待 M5 technical Gate 与 MVR PASS 后 refinement                 |
| P2-M7     | FROZEN    | final state `376d26d`；run `32640294672` 三 jobs 与 8 artifacts 全绿；production `NOT_DEPLOYED`                        |
| P2-M8     | COMMITTED | Golden dataset and integrated evaluation；须待 P2-M5 与 P2-M6 FROZEN 后 refinement                                     |
| P2-M9     | COMMITTED | Final audit and freeze；须待 M1–M8 Gate 后 refinement                                                                  |

M5/M7 受控组合分支只汇合已接受的 M5 current authority 与已冻结 M7 history；它不解除 E01 Owner Decision、M6/M8 entry、production、
