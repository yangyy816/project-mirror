...
`3af4533` passed exact-SHA run `32638417120`, making R14/T08 acceptance effective and the technical Gate `PASS`.
Freeze candidate `7d8e049` then passed exact-SHA run `32639724124` with all eight artifacts inspected, so P2-M7 is
`FROZEN`. This is not an admin API, dashboard, collector, Provider integration or production approval. P2-M5 fresh
study and P2-M6 release/revoke remain closed, so P2-M8 refinement remains closed.
