...
function responseFor(
  result: Awaited<ReturnType<typeof readBoundDemoProfile>>,
  pendingStatus: number,
) {
  const body = profileProjection(result);
...
export async function POST(request: Request) {
  ...
  return responseFor(
    await createBoundDemoProfile(handleFromRequest(request)),
    202,
  );
}
...
export async function GET(request: Request) {
  if (denied(request) || !(await isBodyless(request)))
    return noStoreJson({ code: "DENIED" }, 403);
  return responseFor(
    await readBoundDemoProfile(handleFromRequest(request)),
    200,
  );
}
